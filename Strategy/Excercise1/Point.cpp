...
//Initiliaze the strategy at its creation
std::shared_ptr<DistanceStrategy>Point::strategy = nullptr;
...
//Function to set the strategy of the ptr
void Point::setStrategy(const std::shared_ptr<DistanceStrategy> strategy_type)
{
	 Point::strategy = strategy_type;
}
//Calls the distance function of the current strategy
double Point::distance(const Point& pt) const
{
	
	return strategy->Distance(*this, pt);
}
//Delegate to the second distance function
